...
public class TimeSeries extends TreeMap<Integer, Double> {
    ...
    /**
     * Creates a copy of TS, but only between START_YEAR and END_YEAR,
    ...
    public TimeSeries plus(TimeSeries ts) {
        ...
        TimeSeries sumOfTimeSeries = new TimeSeries();
        for (int year : this.keySet()) {
            if (ts.containsKey(year)) {
                sumOfTimeSeries.put(year, this.get(year) + ts.get(year));
            } else {
                sumOfTimeSeries.put(year, this.get(year));
            }
        }
        for (int year : ts.keySet()) {
            if (!this.containsKey(year)) {
                sumOfTimeSeries.put(year, ts.get(year));
            }
        }
        return sumOfTimeSeries;
    }
